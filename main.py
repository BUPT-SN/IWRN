...
def main():
    ...
    yml_path = '/home/yyh/yyh/PythonProject/IWRN/codes/utils/opt.yml'
    ...
    for current_epoch in range(start_epoch, total_epochs + 1):
        ...
        if opt['train/test'] == 'test':
            ...
            print('main: Test end !')
            break
        if opt['train/test'] == 'validation':
            test_data = utils.test_loader(opt)
            network.validation(test_data, current_epoch)
            print('main: validation end !')
            break
